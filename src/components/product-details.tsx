...
export function ProductDetails({
  ...
  const { product, ready } = useProduct(productId);
  ...
  if (ready && !product) {
    return (
      <div className="py-12 text-center">
        <p className="text-sm text-muted-foreground">No encontramos esta referencia.</p>
        {onClose && (
    ...
    <article className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      {onClose && (
        <div className="flex justify-end">
          <Button variant="ghost" size="sm" onClick={onClose} className="-mr-2 text-muted-foreground">
                        ...
                        {grouped.sizes.map((s) => (
                          <td
                            ...
                            className={
                              ...
                              (row[s] === 0 ? "text-muted-foreground/50" : "text-foreground")
                            }
                          >
                            {row[s] || "—"}
                          </td>
          ...
          <ul className="flex flex-col divide-y divide-border overflow-hidden rounded-2xl border border-border bg-card">
            {product.variants.map((v) => (
              <li key={v.id} className="flex items-center justify-between gap-3 px-4 py-3">
                ...
                <Badge variant={v.stock > 0 ? "default" : "outline"} className="rounded-full tabular-nums">
                  {v.stock} und
                </Badge>
              </li>
            ))}
